...
private:
    ...
    void createInstance() {
        ...
        // After creating queue, retrieve it's handle with :
        //vkGetDeviceQueue(device, indices.graphicsFamily.value(), 0, &graphicsQueue);
    }

    void checkAviableExtensions() {
    ...
    bool isDeviceSuitable(VkPhysicalDevice device) {
        // Can use multiple metrics to select a device or prioritize a more powerful one.
    ...
    void cleanupSwapChain() {
        for (size_t idx = 0; idx < swapChainFramebuffers.size(); idx++) {
            vkDestroyFramebuffer(device, swapChainFramebuffers[idx], nullptr);
        }

        for (size_t idx = 0; idx < swapChainImageViews.size(); idx++) {
            vkDestroyImageView(device, swapChainImageViews[idx], nullptr);
        }
